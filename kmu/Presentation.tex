...
\end{center}
\begin{itemize}
\item \textbf{Divide and Conquer.} A Provably Asymptotically Fast Version of the Generalized Jensen Algorithm for Non-Dominated Sorting. (M. Buzdalov, A. Shalyto)
\item \textbf{ENS-NDT.} A new algorithm using the non-dominated tree to improve non-dominated sorting. (P. Gustavsson, A. Syberfeldt)
\end{itemize}
\end{block}
\end{frame}
...
\item Нам неизвестны ранее опубликованные алгоритмы, которые способны отсортировать любой набор точек размером $10^6$ большой размерности.
